...
export class WatchlistsService {
  ...
  async createPriceAlert(userId: string, alert: {
    ...
    const duplicate = await this.postgres.query<{ id: string }>(
      `select watchlist_items.id::text as id
       ...
       where watchlist_items.user_id = $1
         ...
         and watchlist_items.target_price = $3
       order by watchlist_items.id
       limit 1`,
      [userId, product.product_slug, alert.targetPrice]
    );
    if (duplicate[0]) return this.priceAlerts(userId);
