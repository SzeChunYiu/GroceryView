...
${seo}`;
  ...
  it('surfaces query-driven chain comparison table on the compare route', async () => {
    ...
    assert.match(route, /cheapestChainAnswerRows/);
    assert.match(route, /Cheapest chain answer board/);
    assert.match(route, /Who is cheapest right now/);
    assert.match(route, /delta vs median/);
    assert.match(route, /chainPriceRows\(product\)/);
