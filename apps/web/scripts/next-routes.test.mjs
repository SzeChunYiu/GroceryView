...
${seo}`;
  ...
  it('mounts a glassmorphic mobile bottom navigation in the shared page shell', async () => {
    ...
    assert.match(bottomNav, /href: '\/heatmap'[\s\S]*label: 'Heatmap'/);
    assert.match(bottomNav, /Flame/);
    assert.match(bottomNav, /grid-cols-9/);
  ...
  it('adds common dietary allergen checkbox filters to product listing search', async () => {
    ...
    assert.match(verified, /const avoidAllergens = booleanSearchValue\(searchParams\.avoidAllergens\)/);
