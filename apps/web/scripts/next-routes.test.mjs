...
${seo}`;
  ...
  it('uses crawlable URL query params for instant product search facets', async () => {
    ...
    assert.match(verified, /priceTypeSearchValues/);
    assert.match(verified, /const filters = \{ query, categories, labels, originCountries, chains, priceTypes, minPrice, maxPrice, inStockOnly, minConfidence/);
    ...
    assert.match(products, /priceType\?: string \| string\[\]/);
    ...
    assert.match(products, /name="priceType"/);
  ...
  it('surfaces adaptive total and unit price product cards with a compare-mode toggle', async () => {
    ...
    assert.match(products, /productsPageUrl\(currentPage \+ 1, selectedBrand, resolvedSearchParams, basePath\)/);
    assert.match(shell, /ProductPriceCards/);
    assert.match(shell, /homepageAdaptiveProductCards/);
