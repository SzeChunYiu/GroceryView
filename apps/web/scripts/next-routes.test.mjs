...
${seo}`;
  ...
  it('uses crawlable URL query params for instant product search facets', async () => {
    ...
    assert.match(verified, /resultCount = searchResult\.products\.length/);
    assert.match(verified, /statusMessage/);
    ...
    assert.match(products, /statusMessage=\{search\.statusMessage\}/);
    assert.match(products, /search\.activeFilters/);
    assert.match(categoryFilter, /role="status"/);
    assert.match(categoryFilter, /aria-live="polite"/);
    assert.match(categoryFilter, /facet\.count\.toLocaleString\('sv-SE'\)/);
