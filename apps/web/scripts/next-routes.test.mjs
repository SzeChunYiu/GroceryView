...
${seo}`;
  ...
  it('mounts a glassmorphic mobile bottom navigation in the shared page shell', async () => {
    ...
    assert.match(bottomNav, /href: '\/scanner\?launch=bottom-nav-browser#scan'/);
    assert.match(bottomNav, /standaloneHref: '\/scanner\?launch=bottom-nav-pwa#scan'/);
    assert.match(bottomNav, /label: 'Scan'/);
    assert.match(bottomNav, /grid-cols-7/);
    assert.match(bottomNav, /useHaptic/);
    assert.match(bottomNav, /scannerShortcut\(\)/);
