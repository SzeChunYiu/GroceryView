...
describe('verified-data UI', () => {
  it('ships the requested grouped desktop navigation without legacy personal-product links', async () => {
    ...
    assert.match(nav, /label: t\('app-nav\.groups\.markets'\)[\s\S]*label: t\('app-nav\.items\.overview'\)[\s\S]*label: t\('app-nav\.items\.chainIndex'\)[\s\S]*label: t\('app-nav\.items\.categories'\)[\s\S]*label: t\('app-nav\.items\.heatmap'\)[\s\S]*label: 'Screener'/);
    assert.match(nav, /label: t\('app-nav\.groups\.products'\)[\s\S]*label: t\('app-nav\.items\.browse'\)[\s\S]*label: t\('app-nav\.items\.compare'\)/);
    assert.match(nav, /label: t\('app-nav\.groups\.stores'\)[\s\S]*label: t\('app-nav\.items\.map'\)[\s\S]*label: t\('app-nav\.items\.stores'\)/);
    assert.match(nav, /label: t\('app-nav\.groups\.trip'\)[\s\S]*label: t\('app-nav\.items\.currentList'\)[\s\S]*label: t\('app-nav\.items\.nearbyDeals'\)[\s\S]*label: t\('app-nav\.items\.watchlist'\)/);
    assert.match(nav, /label: t\('app-nav\.groups\.personal'\)[\s\S]*label: t\('app-nav\.items\.savings'\)[\s\S]*label: t\('app-nav\.items\.myFlyer'\)[\s\S]*label: t\('app-nav\.items\.weeklyBasket'\)[\s\S]*label: t\('app-nav\.items\.mealPlanner'\)[\s\S]*href: '\/contact'[\s\S]*label: t\('app-nav\.items\.contact'\)/);
...
${seo}`;
    ...
    assert.match(nav, /app-nav\.tagline/);
