...
${seo}`;
  ...
  it('surfaces product multi-timeframe price charts using the real core chart adapter and lightweight-charts', async () => {
    ...
    assert.match(chart, /priceChartReadoutForWindow/);
    assert.match(chart, /selectedPoint \?\? latestChartPointForWindow/);
    assert.match(chart, /subscribeCrosshairMove/);
    assert.match(chart, /chartPointForCrosshairTime\(activeWindow, param\.time\) \?\? null/);
    assert.match(chart, /volatilityLowerLabel/);
    assert.match(chart, /volatilityUpperLabel/);
    assert.match(chart, /volatilityReadoutForPoint/);
    assert.match(chart, /volatility \$\{point\.volatilityLowerLabel\} → \$\{point\.volatilityUpperLabel\}/);
    assert.match(chart, /volatilityReadoutForPoint\(point\)/);
