...
async function loadProductsRoute() {
  ...
  const executable = source
    ...
    .replace("import { buildStalePriceArchiveQuery, summarizeStalePriceArchive, type StalePriceArchiveSummary } from '@/lib/freshness';", 'const buildStalePriceArchiveQuery = () => ({ cutoffObservedAt: "2026-05-01T00:00:00.000Z", sql: "update latest_prices set is_available = false", thresholdDays: 14, values: ["2026-05-01T00:00:00.000Z"] }); const summarizeStalePriceArchive = (archivedCount, query, status = "archived") => ({ archivedCount, cutoffObservedAt: query.cutoffObservedAt, status, thresholdDays: query.thresholdDays });')
    ...
    .replace(/executor: ReturnType<typeof createPgQueryExecutor>/g, 'executor')
    .replace(/: Promise<StalePriceArchiveSummary>/g, '')
    .replace(/executor\.query<\{ product_id: string \}>/g, 'executor.query')
    .replace(/stalePriceArchive\?: StalePriceArchiveSummary/g, 'stalePriceArchive')
