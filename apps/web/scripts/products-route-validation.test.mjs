...
async function loadProductsRoute() {
  ...
  const executable = source
    ...
    .replace("import { fuzzyProductSearchQueries, rankFuzzyProductResults } from '@/lib/search-fuzzy';", 'const fuzzyProductSearchQueries = (query, expansion) => expansion.expandedQueries; const rankFuzzyProductResults = (_query, batches) => batches.flat();')
    .replace("import { searchExplanationBadgesForProduct } from '@/lib/search-filters';", 'const searchExplanationBadgesForProduct = () => [];')
    .replace("import { expandGrocerySearchQueryWithTelemetry, type GrocerySearchExpansion, type GrocerySearchExpansionTelemetry } from '@/lib/search-suggest';", 'const expandGrocerySearchQueryWithTelemetry = (query) => ({ expansion: { query, expandedQueries: query ? [query] : [], matchedAliases: [], matchedFuzzyAliases: [], matchedSynonyms: [] }, telemetry: { cacheHit: false } });')
