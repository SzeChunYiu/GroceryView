...
export function buildPersonalizedRecommendationRail<T extends RecommendationProductInput>(
  ...
  return products
    .map((product, index) => {
      ...
      let reason = 'Household favorites, watchlist, dietary preferences, and recent searches keep this in the discovery mix';
      if (avoidedHit) {
        reason = `Avoided brand control lowers ${product.brand} for recommendations and substitutions`;
      } else if (learnedBrand?.tolerance === 'excluded') {
        reason = `Ignored recommendation history lowers ${product.brand}: ${learnedBrand.reason}`;
      } else if (learnedBrand?.tolerance === 'favorite') {
        reason = `Learned brand preference for ${product.brand}: ${learnedBrand.reason}`;
      } else if (favoriteHit) {
        reason = `Favorite brand signal for ${product.brand}`;
      } else if (listHits > 0) {
        reason = `${listHits} recent list signal${listHits === 1 ? '' : 's'} matched`;
      }
