...
function checkedStateFromStorage(value: string | null): Record<string, boolean> {
  if (!value) return {};
  ...
  try {
    ...
    if (!maybeCheckedById || typeof maybeCheckedById !== 'object' || Array.isArray(maybeCheckedById)) {
      return {};
    }
...
function withCheckedState(checkedById: Record<string, boolean>): ShoppingListItem[] {
  return baseListItems.map((item) => ({
    ...item,
...
function persistCheckedState(items: ShoppingListItem[]) {
  try {
    ...
    localStorage.setItem(LIST_STORAGE_KEY, JSON.stringify({ checkedById }));
  } catch {
...
export function useList() {
  ...
  useEffect(() => {
    try {
      const checkedById = checkedStateFromStorage(localStorage.getItem(LIST_STORAGE_KEY));
      setItems(withCheckedState(checkedById));
    } finally {
