...
function renderInstallBanner(canInstall: boolean) {
  const existing = document.querySelector<HTMLDivElement>('[data-groceryview-install-banner]');
  ...
  banner.innerHTML = `
    <div class="install-banner__copy">
      <p class="install-banner__eyebrow">Mobile app shortcut</p>
      <strong>Add GroceryView to your home screen</strong>
      <span>${canInstall ? 'Tap Install for a native-like grocery price app that reopens quickly on shopping trips.' : 'Use your browser menu or Share sheet, then choose Add to Home Screen for repeat visits.'}</span>
    </div>
    <div class="install-banner__actions">
      ${canInstall ? '<button class="install-banner__button" type="button">Install</button>' : '<a class="install-banner__button" href="/manifest.webmanifest">Manifest</a>'}
      <button aria-label="Dismiss install banner" class="install-banner__dismiss" type="button">×</button>
    </div>
  `;

  banner.querySelector<HTMLButtonElement>('.install-banner__dismiss')?.addEventListener('click', () => {
    localStorage.setItem(installDismissedKey, 'true');
    banner.remove();
  });
  banner.querySelector<HTMLButtonElement>('.install-banner__button')?.addEventListener('click', async () => {
