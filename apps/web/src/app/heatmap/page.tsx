...
import { indexSymbol, weightedCellIndex } from '@/lib/heatmap-index';
              ...
              {matrixRows.map((row) => (
                <tr key={row.slug}>
                  ...
                  {row.cells.map(({ chain, cell }) => {
                    ...
                    return (
                      <td className="border-b border-slate-200 bg-slate-50 p-1.5" data-heatmap-cell={`${row.slug}:${chain.chainId}`} key={`${row.slug}-${chain.chainId}`}>
                        <Link
