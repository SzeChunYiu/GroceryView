              ...
              <div className="flex gap-3">
                {product.imageUrl ? (
                  <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-white p-2 ring-1 ring-violet-100">
                    <Image alt={`${product.name} product image`} className="max-h-full max-w-full object-contain transition group-hover:scale-105" height={80} sizes="80px" src={toImageProxyUrl(product.imageUrl, { width: 80, height: 80, quality: 80, format: 'webp' })} width={80} />
                  </div>
                ) : null}
                <div>
              ...
              <div className="flex items-start gap-3">
                {product.imageUrl ? (
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-white p-2 ring-1 ring-rose-100">
                    <Image alt={`${product.productName} watchlist product image`} className="max-h-full max-w-full object-contain transition group-hover:scale-105" height={64} sizes="64px" src={toImageProxyUrl(product.imageUrl, { width: 80, height: 80, quality: 80, format: 'webp' })} width={64} />
                  </div>
                ) : null}
          ...
          {immigrantImageFirstBrowsing.map((item) => (
            <Link className="group overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-amber-700" href={`/products/${item.verifiedProductSlug}`} key={item.verifiedProductSlug}>
              <div className="flex aspect-square items-center justify-center bg-white p-3">
                <Image alt={item.visualAlt} className="max-h-full max-w-full object-contain transition group-hover:scale-105" height={160} sizes="(min-width: 768px) 20vw, 50vw" src={toImageProxyUrl(item.imageUrl, { width: 160, height: 160, quality: 80, format: 'webp' })} width={160} />
              </div>
