...
export function generateStaticParams() {
  return seoLandingCities.flatMap((city) => seoLandingProducts.map((product) => ({ city: city.slug, slug: product.slug })));
}
...
export default async function CityCheapestProductLandingPage({ params }: Readonly<{ params: Promise<{ city: string; slug: string }> }>) {
  const { city: citySlug, slug } = await params;
  const city = findSeoLandingCity(citySlug);
  const product = findSeoLandingProduct(slug);
  if (!city || !product) notFound();
