        ...
        <div className="mt-4 space-y-3">
          {watchlistAlerts.map((alert, index) => (
            <WatchlistRow
              ...
              sourceConfidence={confidenceForProduct(alert.productId)}
              sourceLabel={priceSource(alert.productId)}
              sourceSampleSize={priceRowCount(alert.productId)}
              targetPrice={watchlistItemForAlert(alert.productId)?.targetPrice}
            />
          ))}
        </div>
      </Card>
