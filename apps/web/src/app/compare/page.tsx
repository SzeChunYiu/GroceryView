...
import { type ProductGridRow, VirtualizedProductGrid } from '@/components/data-grid';
...
export default async function ComparePage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const productGridRows: ProductGridRow[] = matchedChainProducts.map((product) => ({
    slug: product.slug,
    name: product.name,
    brand: product.brand || 'Brand not reported',
    subline: product.subline || 'No subline reported',
  ...
  return (
    <PageShell>
      ...
      <div className="mt-6">
        <VirtualizedProductGrid rows={productGridRows} />
      </div>
    </PageShell>
  );
}
