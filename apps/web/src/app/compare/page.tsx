...
function resetStoreFiltersHref(products: string | string[] | undefined) {
  const selectedProducts = selectedProductsValue(products).trim();
  return selectedProducts ? `/compare?products=${encodeURIComponent(selectedProducts)}` : '/compare';
}
...
export default async function ComparePage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const noVisibleChains = comparison.storeFilters.visibleChains.length === 0;
  ...
  return (
    <PageShell>
      ...
      <Card className="mt-6 overflow-hidden border-emerald-200 bg-gradient-to-br from-white via-emerald-50 to-sky-50">
        ...
        {noVisibleChains ? (
          <div className="mt-5 rounded-3xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
            <p className="text-sm font-black uppercase tracking-[0.18em] text-amber-800">No chains match these store filters</p>
            <h3 className="mt-2 text-2xl font-black text-amber-950">Coupon, delivery, and pickup filters removed every compare column</h3>
            <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-amber-950">
              Your selected products are still valid, but no chain currently has verified evidence for the active store capability combination. Resetting store filters preserves the selected product ids and shows all compare columns again.
            </p>
            <Link className="mt-4 inline-flex rounded-full bg-amber-900 px-4 py-2 text-sm font-black text-white" href={resetStoreFiltersHref(resolvedSearchParams.products)}>
              Reset store filters
            </Link>
          </div>
        ) : null}
        <div className="mt-5 grid gap-4">
          ...
          {noVisibleChains ? null : rowSections.map((section) => (
