  ...
  return (
    <PageShell>
      ...
      <Card className="mt-6 overflow-hidden border-emerald-200 bg-gradient-to-br from-white via-emerald-50 to-sky-50">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-700">
              ...
              The table uses packages/db snapshot rows when production exports are present and marks missing chain rows explicitly. Coupon, delivery, and pickup filters are driven by generated store capability rows rather than hardcoded UI flags.
            </p>
          </div>
        ...
        <div className="mt-3 rounded-2xl bg-white/85 p-3 text-xs font-semibold leading-5 text-slate-600 shadow-sm">
          {comparison.storeFilters.activeLabels.length > 0
            ? `Showing chains with ${comparison.storeFilters.activeLabels.join(' + ')} evidence. `
            : 'No store-level capability filters are active. '}
          Visible chains: {comparison.storeFilters.visibleChains.map((chain) => chain.label).join(', ') || 'none'}.
          {comparison.storeFilters.visibleChains.map((chain) => ` ${chain.label}: ${chain.evidenceLabel}${chain.evidenceUpdatedAt ? ` (${chain.evidenceUpdatedAt})` : ''}.`).join('')}
        </div>
