        ...
        {visibleRows.map((row) => {
          ...
          return (
            <Link className="group rounded-3xl border border-slate-200 bg-slate-50 p-4 transition hover:-translate-y-0.5 hover:border-emerald-700 hover:bg-white" href={row.href} key={row.id}>
              {row.image ? <div className="mb-4 flex aspect-square w-full items-center justify-center rounded-2xl bg-white p-3"><ProductImage alt={`${row.name} product image`} className="h-full w-full object-contain" src={row.image} /></div> : null}
