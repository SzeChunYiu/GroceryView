"""Hemköp structured JSON scraper stub using the Axfood-shaped parser."""
...
ENDPOINT_TEMPLATE = "https://www.hemkop.se/c/{category}?size=50&page={page}"
...
class HemkopScraper:
    ...
    compliance = SourceCompliance(
        ...
        robots_url="https://www.hemkop.se/robots.txt",
    ...
    async def fetch(self, category: str, page: int = 0) -> RetailerFetchResult:
        raise NotImplementedError("Hemköp network fetching is disabled until legal/data approval.")


def parse_hemkop_products(
    payload: dict[str, Any], *, category: str, source_url: str, observed_at: datetime
) -> RetailerFetchResult:
    """Parse Hemköp/Axfood product JSON into GroceryView observations."""
    ...
    for item in payload.get("results", payload.get("products", [])):
        ...
        product_key = f"hemkop:{code}"

        products.append(
            NormalizedProduct(
                source_type="hemkop_online_json",
        ...
        for promo in item.get("potentialPromotions", []) or []:
            promotions.append(
                PromotionObservation(
                    ...
                    offer_price=(
                        float(promo["rewardPrice"])
                        if promo.get("rewardPrice") is not None
                        else None
                    ),
                    compare_price=(
                        float(promo["lowestHistoricalPrice"])
                        if promo.get("lowestHistoricalPrice") is not None
                        else None
                    ),
                )
            )
