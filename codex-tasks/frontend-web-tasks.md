...
3. [ ] Create the pnpm workspace file at `pnpm-workspace.yaml`.
...
6. [ ] Create root `tsconfig.json` with strict TypeScript defaults.
...
9. [ ] Pin web package versions in `apps/web/package.json`.
...
11. [ ] Initialize shadcn/ui for `apps/web`.
...
12. [ ] Configure `apps/web/next.config.ts`.
    - Set `output: "standalone"`.
    - Keep config minimal and TypeScript-valid.
13. [ ] Create placeholder route structure.
...
14. [ ] Create shared web layout components.
...
15. [ ] Add chart placeholder component.
