...
| Local infrastructure | TODO | db-schema | P0 | `infra/docker-compose.yml` starts a `postgres:18`/PostGIS-compatible service using `postgis/postgis:18-3.6` on port 5432, `redis:7-alpine` on port 6379, optional `pgadmin4` via `dpage/pgadmin4` on port 5050, and S3-compatible object storage; root `.env.example` documents `DATABASE_URL` and `REDIS_URL`; worker docs point to this compose file for real local DB testing. |
