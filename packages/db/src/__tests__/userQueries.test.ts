...
describe('user data export queries', () => {
  it('builds parameterized account-owned export queries for GDPR sections', () => {
    ...
    assert.deepEqual(queries.map((query) => query.section), ['profile', 'lists', 'alerts', 'preferences', 'preferences', 'analytics_events']);
    ...
    assert.equal(queries.filter((query) => query.section === 'preferences').length, 2);
    assert.equal(queries.some((query) => query.section === 'preferences' && /from user_preferences/i.test(query.sql)), true);
    assert.equal(queries.some((query) => query.section === 'preferences' && /from substitution_willingness/i.test(query.sql)), true);
  ...
  it('builds ordered account deletion queries for user-owned lists, alerts, preferences, and profile', () => {
    ...
    assert.match(queries.find((query) => query.table === 'substitution_willingness')?.sql ?? '', /delete from substitution_willingness/i);
