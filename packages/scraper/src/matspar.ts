// This is the public Matspar scraper, used as a supplemental competitor aggregator
// source for pricing/availability gap analysis.
...
export type MatsparCrossCheckReport = {
  query: string;
  ...
  matchedItems: Array<{
    ...
    name: string;
    sourcePrice: number;
    localMatchedName: string;
  }>;
  gapItems: MatsparProduct[];
...
export function buildMatsparCrossCheckReport(
  rows: readonly MatsparProduct[],
  query: string,
  catalogProductNames: readonly string[]
): MatsparCrossCheckReport {
  ...
  const matchedRows: MatsparProduct[] = [];
  const gapRows: MatsparProduct[] = [];

  for (const row of rows) {
    if (normalizedCatalog.has(normalizeSearchKey(row.name))) {
  ...
  const matchedItems = matchedRows.map((row) => ({
    code: row.code,
    name: row.name,
    sourcePrice: row.price,
    localMatchedName: row.name
  }));
  ...
  return {
    ...
    matchedItems,
    gapItems: gapRows,
    catalogCandidates: catalogProductNames.length
  };
}
