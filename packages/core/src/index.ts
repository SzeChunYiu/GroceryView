...
function priceMatchesBasketScope(price: StorePrice, input: BasketComparisonInput) {
...
export function compareBasketStrategies(input: BasketComparisonInput): BasketComparisonResult {
  ...
  for (const item of input.items) {
    const favoritePrices = item.prices.filter((price) => favoriteSet.has(price.storeId) && priceMatchesBasketScope(price, input));
...
export function summarizeStoreBasketCoverage(input: BasketComparisonInput): StoreBasketCoverageSummary {
  ...
  for (const item of input.items) {
    for (const storeId of input.favoriteStoreIds) {
      ...
      const price = item.prices.find((candidate) => candidate.storeId === storeId && priceMatchesBasketScope(candidate, input));
      if (!price) {
