...
function assertObIsSourceUrl(sourceUrl: string) {
  const host = new URL(sourceUrl).hostname;
  if (host !== 'olis.ob.is' && host !== 'www.ob.is' && host !== 'ob.is') {
    throw new Error('OB IS fuel connector only accepts ob.is source URLs');
  }
}
...
export function parseObIsFuelPricePage(input: {
  ...
  assertObIsSourceUrl(sourceUrl);
  if (/captcha|access denied|innskráning/i.test(input.body)) throw new Error('OB IS fuel source blocked/login page');
  ...
  return fuelHeaders.flatMap((fuel) => {
    const row = bestByGrade.get(fuel.productId);
    return row ? [row] : [];
  });
}
...
export async function fetchObIsFuelPrices(options: {
  ...
  const response = await fetchImpl(sourceUrl, {
    headers: {
      ...
      'user-agent': 'GroceryView/0.1 ob-is-connector (fixture-friendly)'
    }
  });
  if (!response.ok) {
