...
describe('createGroceryViewApi', () => {
  ...
  it('serves market overview, product search, and product details', () => {
    ...
    assert.deepEqual(detail?.currentPrices[0].provenance, {
      sourceName: 'Willys campaign page',
      sourceUrl: 'https://www.willys.se/',
      rawRecordId: 'demo-price-coffee-willys-20260519'
    });
    assert.equal(detail?.currentPrices[0].priceType, 'promotion');
    assert.equal(detail?.currentPrices[0].confidence, 'verified');
    assert.equal(detail?.currentPrices[0].observedAt, '2026-05-19T08:30:00.000Z');
    assert.equal(detail?.currentPrices[0].sourceType, 'retailer_page');
