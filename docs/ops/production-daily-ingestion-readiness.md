...
4. production ingestion configuration validator and always-attempted `groceryview-production-ingestion-config` artifact upload with `production-env-validation.json`, `groceryview-catalog-targets.json`, and `groceryview-daily-connectors.json` for success or failure diagnostics; if daily connector generation exits before writing JSON, the workflow writes a fail-closed `daily_connectors_diagnostic_missing` payload with the command exit code
