...
`/tmp/daily-db-io-hotspots-after.json`, then comparing shared `queryid` rows in
`/tmp/daily-db-io-hotspots-delta.json`. The delta artifact reports before,
after, and delta values for `sharedBlks*`, `localBlks*`, `tempBlks*`, and block
read/write timing counters, plus before-only and after-only query ids. All three
files are uploaded in the `groceryview-daily-db-io-hotspots` artifact. Empty
`hotspots[]` rows are valid evidence that no block IO rows were present in
`pg_stat_statements`; the comparison still emits a zero-row delta instead of
failing on empty before/after arrays. A workflow failure should mean a diagnostic
file was missing or the query itself failed.
...
10. daily DB IO hotspot delta comparison and always-attempted `groceryview-daily-db-io-hotspots` artifact upload with `daily-db-io-hotspots-before.json`, `daily-db-io-hotspots-after.json`, and `daily-db-io-hotspots-delta.json`; empty before/after `hotspots[]` arrays produce a valid zero-row comparison, and if either input file is missing the delta records `daily_db_io_hotspots_delta_missing_before_or_after`
...
- `daily_db_io_hotspots_delta_missing_before_or_after`: the daily DB IO hotspot comparison could not find both before and after capture files; inspect `groceryview-daily-db-io-hotspots` to find which capture failed or was skipped.
