...
- Added this iteration (worker B fourteenth follow-up): 246549 Coop Vittangi, 13 rows; 246550 Coop Svappavaara, 14 rows; 245030 Coop Övertorneå, 17 rows; 085130 Coop Mönsterås, 21 rows; 085320 Coop Hultsfred, 15 rows. Store API validity and flyer URLs came from matching `https://proxy.api.coop.se/external/store/stores/<storeId>?api-version=v5&includeFlyers=true&onlyVisibleOpeningHours=true` responses retrieved at 2026-05-23T19:59:28.767Z. Ordinary and offer prices came from matching store-scoped `https://external.api.coop.se/personalization/search/products?store=<storeId>&device=desktop&direct=true&api-version=v1` responses; only product-search rows with ordinaryPrice greater than offerPrice were emitted. The direct personalization probe inspected all 591 not-yet-ingested Coop branches and found no other qualifying ordinary-price-greater-than-offer-price rows in this pass.
- Added this iteration (worker B fifteenth follow-up): 79 additional real Götene/Hushållsost weekly flyer discount rows across already-covered Coop branches (source rows retrieved at 2026-05-23T21:16:17.906Z). Store API validity and flyer URLs came from matching `https://proxy.api.coop.se/external/store/stores/<storeId>?api-version=v5&includeFlyers=true&onlyVisibleOpeningHours=true` responses, and ordinary/offer prices came from matching store-scoped `https://external.api.coop.se/personalization/search/products?store=<storeId>&device=desktop&direct=true&api-version=v1` responses. Only product-search rows with ordinaryPrice greater than offerPrice were emitted.
- Stores:
