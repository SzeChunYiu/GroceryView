...
- Real OCR/camera/upload pipeline for barcode and receipt scanning. Provider-neutral scan pipeline and protected scan-processing API bridge exist; real camera/OCR providers, durable upload storage, and live provider credentials still missing.
...
- Real interactive web UI for login, account, household, privacy, basket, and scanner flows. Static page scaffolds, an account subscription-access panel, a scanner review desk, provider-safe client-side flow actions, an API session bridge for account alert plus basket writes, protected privacy export/deletion-plan routes, scanner API processing bridge, and household plan API bridge exist; real auth-provider session exchange, durable database-backed household/UI state, and provider-backed upload/session flows remain incomplete.
