...
- Hosting provider selection, real deployment, secrets, DNS, observability, smoke tests, and rollback. Deployment readiness/rollback gates, scheduled-worker checks, a server runtime entrypoint for the manifest start command, runtime repository/sink wiring hooks, PostgreSQL pool bootstrap from `DATABASE_URL`, and a token-protected PostgreSQL readiness endpoint exist; real provider deployment, production environment secrets populated in GitHub, a provisioned migrated PostgreSQL service, and successful hosted smoke workflow evidence still missing.
