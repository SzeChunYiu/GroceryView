...
- AdMob/AdSense and subscription billing integration. Provider-neutral monetization contracts, provider-readiness gates, subscription entitlement persistence, entitlement access policy, an account subscription-access API, and repository-backed entitlement lookup exist; real provider credentials, webhook endpoints, provider-specific adapters, live checkout/ad-serving proof, production repository injection, and interactive account UI enforcement still missing.
