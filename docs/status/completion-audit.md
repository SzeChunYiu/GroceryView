...
| --- | --- | --- |
| Iterative PR + merge workflow | PR #1 through PR #36 and PR #88 through PR #104 merged to `main` after this iteration | Done for shipped iterations |
...
- Push/email provider adapters and production notification workers. Provider-neutral delivery, worker tick orchestration, persisted task schedules, suppression filtering, persisted suppression records, suppression event normalization, and a signed suppression webhook route exist; real provider credentials, deployed workers, provider-specific signature adapters, and observability still missing.
