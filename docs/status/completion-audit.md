...
- Real OCR/camera/upload pipeline for barcode and receipt scanning. Provider-neutral scan pipeline, protected scan-processing API bridge, and protected upload-ticket bridge exist; real camera capture, real object-storage provider credentials, actual browser upload transfer, OCR providers, and live provider credentials still missing.
