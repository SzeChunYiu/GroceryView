...
| Connected product terminal UI | This PR, `apps/web/scripts/pages.mjs`, `apps/web/public/styles.css`, and web page-generation tests wire the coffee product page to fetch `/api/products/{id}/terminal` from the configured API base and render live quote, Stockholm/local distribution, history, and chart-series summaries without unsafe HTML injection | Completed by this PR merge |
