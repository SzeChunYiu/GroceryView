...
| PostgreSQL repository smoke probes | Iteration 65, `buildPostgresRepositorySmokeProbes` | Product-specific read/write probe definitions shipped |
...
- PostgreSQL query-executor wiring, repository adapter skeleton, human-review assignment persistence, a schema/migration/probe readiness contract, and repository smoke probe definitions exist; live database integration tests still need a provisioned PostgreSQL service and CI probe runner.
...
- AdMob/AdSense and subscription billing integration. Provider-neutral monetization contracts and provider-readiness gates are being added; real provider credentials, webhook endpoints, provider-specific adapters, and live checkout/ad-serving proof still missing.
