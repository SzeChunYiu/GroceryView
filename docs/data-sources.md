...
- **Current generated slice:** at least 900 products from the live API when stable, with a fallback to the existing OpenPrices/OpenFoodFacts-backed module if the API cannot return the guarded tranche.
...
2. **OpenFoodFacts catalog widen** — current route/module exposes a guarded >=900-row metadata-only slice; completing the full ~25,090-product Swedish catalog still needs a resilient export/filter path because the public search API 503s during full pagination.
