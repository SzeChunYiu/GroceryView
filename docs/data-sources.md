...
### 1.1 OpenStreetMap — Overpass API ✅ shipped (PR #528)
- **Endpoint:** `https://overpass-api.de/api/interpreter` (POST)
- **Query:** `[out:json];area["name"="Stockholms län"]->.a;(node["shop"~"^(supermarket|convenience|grocery)$"](area.a);way[...](area.a););out tags center;`
- **Headers:** `User-Agent: <project>/0.1 (contact)` (UA is mandatory — anonymous requests return 406)
- **What it returns:** 1,041 Stockholm county supermarket+convenience nodes with `name`, `brand`, `shop`, `addr:*`, `lat`/`lon`, `opening_hours` (~30% coverage), `services`.
...
- **Lands in:** `apps/web/src/lib/osm-stores.ts`; route `/stores`.
- **Refresh script:** `apps/web/scripts/refresh-osm-stores.mjs` (TO BUILD).

### 1.2 Nationwide OSM ⏳ pending
- Same endpoint, swap `Stockholms län` → `Sverige` for ~10,000+ stores nationwide.
...
2. **Nationwide OSM stores** — widen from Stockholm to all of Sweden (~10,000 stores).
3. **OpenFoodFacts catalog widen** — pull all 25,059 Swedish products into a slim metadata module (no prices) so the catalog dimension is complete.
4. **Refresh scripts** — `apps/web/scripts/refresh-osm-stores.mjs`, `refresh-axfood-prices.mjs`, `refresh-openprices.mjs` so cron can rerun.
5. **Coop discovery** — find the real `coop.se/handla` JSON endpoint via headless browser inspection.
6. **Mathem** — try `mathem.se/sv-se/api/products/search/` (with trailing slash) and inspect their `_next/data/` payload for catalog + postcode prices.
7. **Receipt scanner ground-truth** — finish the `packages/scanning` connector so user-submitted receipts populate per-branch price rows.
