...
describe('daily ingestion persistence SQL', () => {
  it('deduplicates exact connector observation replays and latest-price conflict keys within each batch', () => {
    ...
    assert.match(dbSource, /partition by\s+product_id,\s*chain_id,\s*store_id,\s*domain,\s*price_type,\s*channel,\s*observed_at,\s*retailer_product_ref,\s*price,\s*unit_price,\s*currency,\s*is_available,\s*confidence,\s*provenance/is);
    assert.match(dbSource, /observations\.domain = ranked_input\.domain/);
    assert.match(dbSource, /observations\.channel = ranked_input\.channel/);
    ...
    assert.match(dbSource, /distinct on \(\s*product_id,\s*chain_id,\s*store_id,\s*price_type,\s*channel\s*\)/);
    assert.match(dbSource, /from written\s+order by\s+product_id,\s*chain_id,\s*store_id,\s*price_type,\s*channel,\s*observed_at desc/is);
  });
